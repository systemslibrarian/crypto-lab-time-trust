 ...
 *
 *  - TEXT OVER A SURFACE AXE DECLINES TO RESOLVE. Every verdict this lab
 *    reaches lands on one: `.integrity.is-ok` and `.integrity.is-alarm` are
 *    `rgba()` fills over a `--panel-2` subcard over a `--panel` card over the
 *    page, and `.chip-math.is-bad` is another — which means the three surfaces
 *    the whole lab exists to put side by side (the cryptographic result, the
 *    security verdict, and the integrity line saying they disagree) are exactly
 *    the ones a violations-only gate could not measure. The tour banner
 *    (`color-mix(in oklab, var(--accent) 10%, var(--panel))`), the tour CTA
 *    (`color-mix(… 22%, var(--panel-2))`) and the hero aside
 *    (`color-mix(… 6%, transparent)`) are three more.
 ...
 *    boxes are `.totp-scroll` (`overflow-x: auto`), which genuinely overflows at
 *    phone width because its seven acceptance windows are each
 *    `flex: 1 0 6.4rem`; the two `.timeline-wrap`s; `.hexdump`, which holds a
 *    full DER certificate; and `.visually-hidden`, handled separately below by
 *    the `clip` guard. Content scrolled past
 ...
 *    HAS an SVG figure — the master timeline, whose `<text>` labels are drawn
 *    over the `<rect>` validity bars — and it is exactly the shape this branch
 *    exists for: an ancestor-only walk would composite those labels onto the
 *    `<svg>`'s own transparent background rather than onto the bar beneath them,
 *    and report a ratio nothing on screen has. In practice the walk skips that
 *    subtree anyway because the `<svg>` is `aria-hidden="true"`, which is why
 *    `gate.ts` measures its parts separately in `auditTimelineGraphics`; the
 *    branch stays for the day that changes.
 *
 ...
 * marker — the stylesheet sets `cursor`, `font-weight`, `font-size` and `color`
 * on `summary` and nothing else — so unlike most labs in this fleet there is no
 * author-painted `?` or `▸` glyph carrying the affordance.
 *
 ...
 * than incidental: the master timeline `<svg>` is `aria-hidden="true"`, and it
 * is the lab's headline visualization. Its graphic parts — the validity bars,
 * the NOW cursor, the axis and row labels — are measured by
 * `auditTimelineGraphics` in `gate.ts` at every driven state, precisely because
 * this walk and axe both stop at the attribute.
 */
 ...
 * uses. Two things here are genuinely short-lived: `.panel-flash`, which
 * `lab.flashAlarm()` puts on a whole `<section>` for 0.9s whenever a preset or a
 * threshold crossing produces a wrong verdict, and `.step.is-active`, which the
 * certificate stepper walks across four cards at 420ms each. Under the reduced
 * motion this gate asserts, the stylesheet cancels the first outright
 * (`animation: none`) so it paints nothing transient — and the second is NOT an
 * animation at all but a JS-driven class, which is why the drive scans it while
 * it is on screen and then waits for its own end condition.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 420ms is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * clock change rebuilds a full result block in seven panels — math chip,
     * verdict chip, a `<ul class="checks">` of two to four rows and an integrity
     * line each — beside a seven-cell TOTP strip, a five-column node matrix and
     * a DER hex dump, so a single state is hundreds of sibling elements all
     ...
     * that is `.totp-scroll` and both `.timeline-wrap`s (`overflow-x: auto`),
     * the `.hexdump` that holds a full DER certificate, and `.visually-hidden`,
     * which is `overflow: hidden` at 1x1 px — the last is caught by the
     * zero-area `clip` guard below as well, and belt-and-braces is right here
     * because this lab uses that idiom four times, once for a paragraph that
     * describes the entire timeline.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // disclosures that ship shut: the intro's longer framing and the
      // certificate panel's DER-bytes expert section. The gate opens each by
      ...
      // replaces did to both of them at once, before its single scan.
      ...
      //
      // THIS LAB USES THE CLIP IDIOM FOR REAL, four times, which is why the
      // guard is load-bearing here rather than inherited. `style.css` defines
      // `.visually-hidden` as `position:absolute; width:1px; height:1px;
      // overflow:hidden; clip:rect(0 0 0 0)`, and it carries the prose
      // alternative for the `aria-hidden` timeline SVG plus the `<caption>` of
      // all three tables. Each has a rect and passes `checkVisibility()` while
      // the compositor draws exactly zero pixels of it, so a walk without this
      // guard reports ratios for ink that was never laid down — and the timeline
      // summary in particular is a long paragraph that would report a fresh
      // fabricated failure on every scan. It is deliberately narrow: only a
     ...
     *
     * What this lab hides is a closed set with three members. Two are plainly
     * decorative: the ✓/✗/⚠ glyphs `verdict.ts` puts in
     * `<span aria-hidden="true">` before each integrity line and check row —
     * the non-colour cue, with the words that say the same thing immediately
     * beside it in the same ink — and the shared header's two SVG marks.
     *
     * THE THIRD IS NOT DECORATIVE AND IS STATED PLAINLY: the master timeline
     * `<svg>`. It is `aria-hidden` for a good reason (a `.visually-hidden`
     * paragraph gives every validity window in prose, which is a better
     * alternative than a screen reader crawling through `<text>` nodes), but the
     * consequence is that the lab's headline visualization sits in the one place
     * BOTH oracles stop looking. It is therefore measured on purpose, by
     * `auditTimelineGraphics` in `gate.ts`, at every driven state.
     *
     * Nothing here hides a VALUE. No verdict, no clock reading, no signature
     * byte and no counter is inside an `aria-hidden` subtree — which was checked
     * rather than assumed, because that is the shared blind spot where BOTH
     * oracles stop, and it is the one place a live readout can hide from a whole
     * accessibility gate.
     */
     ...
     *
     * This lab builds a real SVG figure — `clockPanel.ts`'s `drawTimeline()`
     * appends `<line>`, `<rect>` and `<text>` through `createElementNS` — so the
     * guard is live rather than hypothetical. The mistake it catches is
     * available here in one line: `svgEl()` takes only attributes, and a caller
     * who appends a label to the `<svg>` or a `<g>` instead of to a `<text>`
     * gets a node that is in the DOM, has a rect, and paints nothing, which a
     * walk without this guard would report as a 1:1 failure for ink never laid
     * down.
     */
     ...
     * assumed: `html` sets `background-color: #0b1418` (and `#f2f8fa` under
     * `[data-theme=light]`), `body` sets `var(--bg)`, and neither sets a height,
     * so `<html>`'s box already spans the document and the geometry-aware walk
     * would have found it anyway. It is kept because this page runs nine
     * sections deep and the day a `height: 100%` appears on either box —
     * or the day `html`'s literal hex is dropped as redundant with `body`'s —
     * everything below the first screen would report against a white page that
     * does not exist, in a lab whose dark palette is `#0b1418`.
     */
