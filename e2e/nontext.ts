 ...
 *
 * WHAT THIS FINDS ON TIME TRUST, and what it deliberately does not.
 *
 ...
 * measured separately by `auditTimelineGraphics` in `gate.ts`: the master
 * timeline `<svg>`. It is `aria-hidden="true"` — correctly, since a
 * `.visually-hidden` paragraph states every validity window in prose — and its
 * parts are shapes rather than controls, so neither the `CONTROL` list below nor
 * `contrast.ts` reaches them. Yet the validity bars, the NOW cursor and the row
 * labels ARE "parts of a graphic required to understand the content": they are
 * the whole argument the lab is making.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks seven panels, each of which rebuilds a full
     * result block — math chip, verdict chip, a `<ul class="checks">` of two to
     * four rows, and an integrity line — on every clock change, plus a
     * seven-cell TOTP strip, a five-column node matrix and a DER hex dump.
